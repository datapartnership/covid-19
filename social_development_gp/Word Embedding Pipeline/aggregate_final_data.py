"""
Module to aggregate all not aggregated final twitter data
Developed by :
...
DATA_PATH  = os.path.join(SCRIPT_PATH, 'Dataset/Final/')
...
if __name__=='__main__':
	...
	for filename in filenames:
		...
		df[[col for col in df.columns if col not in ['tweet_id','user_id','created_at','location','text','sentiment_score','verified','reply_count','retweet_count','favorite_count','closest_news_id_keyword_expanded','closest_news_id_keyword_laser']]] = df[[col for col in df.columns if col not in ['tweet_id','user_id','created_at','location','text','sentiment_score','verified','reply_count','retweet_count','favorite_count','closest_news_id_keyword_expanded','closest_news_id_keyword_laser']]].fillna(value=0)
	...
	df.to_csv(DATA_PATH+'agg_final_data.csv',index=False) 
