...
def main():
	...
	#Load Model
	model_all = gensim.models.Word2Vec.load('/mnt/louis/Issue Monitoring/model/w2v_all_300.model')
	...
	df.to_csv('/mnt/louis/Issue Monitoring/TOP_10_Similar_Issue_Keyword.csv',index=False)
	...
	for topic in tqdm(list(df['topic'].unique())):
		...
		for date in list(df_tweets['created_date'].unique()):
			cnt = 0
			df_filter_date = df_tweets[df_tweets['created_date']==date]
	...
	df_topic_daily.to_csv('/mnt/louis/Issue Monitoring/daily_keyword_count.csv',index=False)
	...
	for topic in tqdm(list(df['topic'].unique())):
		...
		for week in list(df_tweets['created_week'].unique()):
			#Load Model
	...
	df_keyword_evolution.to_csv('/mnt/louis/Issue Monitoring/TOP_10_Evolution_Weekly_Similar_Issue_Keyword.csv',index=False)
	...
	df_topic_daily_new_normal.to_csv('/mnt/louis/Issue Monitoring/daily_keyword_count_new_normal.csv',index=False)
